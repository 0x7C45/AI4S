# Phase 3: 通用化 + 离线打包 - Context

**Gathered:** 2026-07-14
**Status:** Ready for planning
**Source:** Derived from ROADMAP Phase 3 + CONSTRAINTS §11/§12 + Phase 1/2 SUMMARY + SMOKE_TEST_REPORT

<domain>
## Phase Boundary

Phase 3 交付：将 Phase 1/2 的 case1 验证管道通用化到 10 个隐藏电路，补全离线打包（wheelhouse + THIRD_PARTY.md + README），死码过滤精化，5000 笔性能优化，Docker 复验。最终产出可提交的 run.sh 独自跑通的完整包。

**Phase 3 不做**：新增覆盖率采集逻辑（Phase 2 已完成）、新增 testbench 骨架（Phase 1 已完成）。
...
### 死码过滤精化（Phase 2 遗留）
- Phase 2 dead_code_analyzer 对 if/else 块边界识别不精确（把 else 激活分支也标死）
- Phase 3 精化：只标记 `if (PARAM)` 到对应 `else` 之间的行为死码，`else` 块保留为可达
- 或保守方案：死码过滤改为可选（默认关闭），覆盖率分母含全部行（保守真实）

### 5000 笔性能优化（Phase 2 遗留）
- Phase 2 case1 5000 笔 + 覆盖率插桩超 10 分钟 timeout
...
- `requirements.txt`：每包钉死 ==X.Y.Z（Phase 1 已建）
...
- `THIRD_PARTY.md`：版本/许可/调用边界（已存在，Phase 3 确认完整性）
- `README.md`：离线安装运行说明
...
- Phase 3 重点检查 #2/#5：框架不可含 case1-5 的特定 RTL 路径/参数/bin 硬编码
...
### Phase 1/2 产出（Phase 3 基础）
- `E:/AI4S/Track-A/A2-verification/run.py` — 完整流水线（Phase 3 优化 timeout）
- `E:/AI4S/Track-A/A2-verification/src/sim_runner.py` — Docker 仿真（Phase 3 调 timeout）
- `E:/AI4S/Track-A/A2-verification/src/dead_code_analyzer.py` — 死码分析（Phase 3 精化）
...
- `E:/AI4S/Track-A/A2-verification/THIRD_PARTY.md` — 第三方依赖（Phase 3 确认完整）
