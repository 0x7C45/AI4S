...
| Berkeley ABC | Yosys 0.54 捆绑版本 | BSD-style | 由 Yosys `abc` pass 调用 | 组合逻辑优化与 Nangate45 工艺映射 | 否 |
...
提交源码没有复制 Yosys 或 Berkeley ABC 源码，也不包含它们的二进制。工具仅通过本地进程边界调用评测镜像中已安装的 Yosys。`src/` 只使用 Python 标准库，不下载或安装依赖。
